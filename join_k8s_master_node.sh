...
echo "📥 Installing Kubernetes kubelet, kubeadm, kubectl v${K8S_VERSION}..."
...
sudo kubeadm join ${MASTER_API_SERVER} \
  --token ${TOKEN} \
  --discovery-token-ca-cert-hash ${DISCOVERY_HASH} \
  --control-plane --certificate-key ${CERT_KEY}

echo "🎉 Node successfully joined as a master to the Kubernetes cluster!"
